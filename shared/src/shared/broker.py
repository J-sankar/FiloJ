...
class BrokerClient:
    ...
    async def connect(self, prefetch_count: int = 1):
        ...
        self.connection = await connect_robust(
            self.rabbitmq_url, reconnect_interval=5, timeout=10, fail_fast=False
        )
    ...
    async def setup_exchanges(self):
        ...
        self.exchanges["work.tasks"] = await self.channel.declare_exchange(
            name="work.tasks", type=ExchangeType.TOPIC, durable=True
        )

        self.exchanges["system.events"] = await self.channel.declare_exchange(
            name="system.events", type=ExchangeType.TOPIC, durable=True
        )
        self.exchanges["webhook.retry"] = await self.channel.declare_exchange(
            name="webhook.retry",
            type="x-delayed-message",
            arguments={"x-delayed-type": "topic"},
            durable=True,
        )
        self.exchanges["dlx.exchange"] = await self.channel.declare_exchange(
            "dlx.exchange", type="topic", durable=True
        )
        logger.debug(
            f"Exchange setup: Success | found exchanges = {len(self.exchanges)}"
        )

    async def get_configured_queue(
        self, exchange_name: str, routing_key: str, queue_name: str
    ) -> AbstractQueue:
        """
        ...
        logger.info(
            f"queue {queue_name} setup and binds exchange {exchange_name} | routing key: {routing_key}"
        )
        return queue
